    ...
    public function cotizar(){
        if($this->recaptcha()){          
            if(!empty($_POST['Nombre']) && !empty($_POST['Email']) && !empty($_POST['noPuntos'])){                 
                $texto='La persona: '.$_POST['Nombre']. ', ha solicitado una cotización de instalación eléctrica,  para '.$_POST['noPuntos'].' puntos, en un edificio de '.$_POST['noNiveles'].' nivele(s), con un área de '.$_POST['Area'].' Enviar cotización al correo: '.$_POST['Email'].' o comunicarse al teléfono: '.$_POST['Telefono'];
