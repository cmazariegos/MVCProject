    ...
    public function cotizar($modulo){
        if($this->recaptcha()){        
            if(!empty($_POST['Nombre']) && !empty($_POST['Email'])){
                $texto='La persona: '.$_POST['Nombre']. ' ,ha solicitado una cotización del modulo: '.$modulo.', del sistema ERP, para '.$_POST['Usuarios'].' usuarios, con sistema '.$_POST['Sistema'].' y servidor de aplicaciones '.$_POST['Servidor'].'. Enviar cotización al correo: '.$_POST['Email'].' o comunicarse al teléfono: '.$_POST['Telefono'];
